// IMPLEMENTACION DEL CASO DE USO PARA OBTENER LAS NOTAS

import 'package:equatable/equatable.dart';
import 'package:note_app/src/core/type_defs/type_defs.dart';
import 'package:note_app/src/core/use_cases/use_cases.dart';
import 'package:note_app/src/features/notes/domain/entities/note_entity.dart';
import 'package:note_app/src/features/notes/domain/respository/note_repository_interface.dart';

class GetNotesUseCase
    implements UseCaseInterface<List<NoteEntity>, GetNoteParams> {
  GetNotesUseCase({required this.noteRepositoryInterface});

  final NoteRepositoryInterface noteRepositoryInterface;

  @override
  FutureEither<List<NoteEntity>> call(GetNoteParams paramsType) async {
    final result = await noteRepositoryInterface.getNotes(
      userId: paramsType.userId,
      query: paramsType.query,
    );
    return result;
  }
}

class GetNoteParams extends Equatable {
  const GetNoteParams({required this.userId, required this.query});

  // PARAMETROS REQUETRIDOS
  final int userId;
  final String? query;

  @override
  List<Object?> get props => [];
}
